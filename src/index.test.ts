...
describe('personnummer', () => {
  it('should validate personnummer with control digit', async () => {
    ...
    for (const item of list.filter((item) => item.valid)) {
      for (const format of availableListFormats) {
  ...
  it('should parse personnummer', async () => {
    ...
    for (const item of list.filter((item) => item.valid)) {
      for (const format of availableListFormats.filter(
        (f) => f !== 'short_format'
      )) {
